...
if (OC.config.version.substring(0,2) >= 28) {
	...
	function registerAction(ext, attr)
	{
		registerFileAction(new FileAction({
			...
			enabled(nodes) {
				...
				const node = nodes[0]
				const name = String(node.basename || node.filename || node.name || '').toLowerCase()
				const isMboardByMime = attr.mime === node.mime
				const isMboardByExt = name.endsWith('.mboard')
				return (isMboardByMime || isMboardByExt) && (node.permissions & OC.PERMISSION_READ) !== 0
			},
	...
	function addMenuEntry(ext, attr)
	{
		const entry = {
			...
			handler(file, folder)
			{
				...
				if (!window.OC.getCurrentUser().uid) {
					...
					const fileName = "MultiBoard " + new Date(Date.now()).toUTCString().substring(5).replace(/\:/g, "-") + ".mboard";
					const webDavPath = "/remote.php/dav/files/" + userId + normalizedPath + fileName;
					var webDavUrl = window.OC.getProtocol() + "://" + window.OC.getHost() + window.OC.getRootPath() + encodeURI(webDavPath);
					$.ajax({
						url: webDavUrl, 
						method: 'PUT', 
		...
		try {
			...
			// Fallbacks for NC variants where wrapper validation is stricter.
			try {
				if (window._nc_newfilemenu?.registerEntry) {
					window._nc_newfilemenu.registerEntry(entry)
					return
				}
			} catch (e2) {
				// Last-resort fallback: append entry directly.
				if (Array.isArray(window._nc_newfilemenu?._entries)) {
					if (!window._nc_newfilemenu._entries.find((x) => x.id === entry.id)) {
						window._nc_newfilemenu._entries.push(entry)
					}
					return
				}
				throw e2
